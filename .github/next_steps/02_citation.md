...
It is likely that your `CITATION.cff` currently doesn't pass validation. The error messages you get from the [`cffconvert`](https://github.com/abelsiqueira/python-template-test/actions/workflows/cffconvert.yml) GitHub Action are unfortunately a bit cryptic, but doing the following helps:
...
To help you keep the citation metadata up to date and synchronized, the [`cffconvert`](https://github.com/abelsiqueira/python-template-test/actions/workflows/cffconvert.yml) GitHub Action checks the following 6 aspects:
